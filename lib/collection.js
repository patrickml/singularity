...
export class Collection {
   ...
   */
  constructor({ name, model, collection }) {
    const Model = model || _Model;

    Model.prototype.getCollection = () => this;

    this._collection = collection || new Mongo.Collection(name, {
      /**
   ...
   */
  getRawCollection(direct = false) {
    return !direct && this._collection || this._collection.direct;
  }
   ...
   * @return {Promise}
   */
  update(query = {}, doc = {}, multi = {}, direct = false) {
    // create a new promise so we can `update` asynchronously
    return new Promise((resolve, reject) => {
      // insert our document
      this.getRawCollection(direct).update(query, doc, multi, (error, result) => {
        if (error) {
          reject(error); // if we have an error return the error
        } else {
          resolve(result); // on success return the result, this should be an _id
          // if there is an afterUpdate method call it
          if (this.afterUpdate) this.afterUpdate(document, result);
        }
      });
    });
  }
  ...
  /**
   * Return a single document from the database synchronously
   * @param  {Object} query    MongoDB query
   * @param  {Object} selector MongoDB field selectors
   * @return {Object}
   */
  firstSync(query = {}, selector = {}) {
    return this.getRawCollection().findOne(query, selector)
  }
