...
export class Query {
  ...
  /**
   * Build a where query if check is true
   * @param  {String} key        the key to look up
   * @param  {String} is         the opertator to use
   * @param  {String} value      the value we want to find
   * @return {Query}             the instance
   */
  whereIf(key, is, value, check) {
    if (check()) {
      this.where(key, is, value);
    }
    return this;
  }
  ...
  /**
   * Find documents with a value between two values if check is true
   * @param  {Function} callback  the operations you want to make with the `$or`
   * @return {Query}              instance of the builder
   */
  betweenIf(key, start, end, check) {
    if (check()) {
      this.between(key, start, end);
    }
    return this;
  }
  ...
  /**
   * Returns all geonears
   * @return {Array} the geonears
   */
  getGeoNears() {
    return this.geoNears;
  }
